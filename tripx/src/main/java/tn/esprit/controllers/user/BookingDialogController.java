...
public class BookingDialogController {
    ...
    public void preSelectActivity(Activity activity) {
        // Called after setDestination() — pre-select the activity in the combo
        if (activity == null || activityCombo == null) return;
        for (Activity a : activityCombo.getItems()) {
            if (a.getActivityId() != null && a.getActivityId().equals(activity.getActivityId())) {
                activityCombo.setValue(a);
                break;
            }
        }
        // If not in list yet, still select it
        if (activityCombo.getValue() == null) {
            activityCombo.getItems().add(activity);
