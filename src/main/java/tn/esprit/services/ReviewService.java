...
public class ReviewService {
    ...
    public boolean addReview(Review review) {
        String sql = "INSERT INTO reviews (user_id, destination_id, rating, comment) VALUES (?, ?, ?, ?)";
        try {
            PreparedStatement ps = cnx.prepareStatement(sql);
            ps.setInt(1, review.getUserId());
            ps.setLong(2, review.getDestinationId());
            ps.setInt(3, review.getRating());
            ps.setString(4, review.getComment());

            int rows = ps.executeUpdate();
            if (rows > 0) {
                recalculateAverageRating(review.getDestinationId());
                return true;
            }
            return false;
    ...
    public List<Review> getReviews(long destinationId) {
        List<Review> reviews = new ArrayList<>();
        String sql = "SELECT * FROM reviews WHERE destination_id = ? ORDER BY created_at DESC";
        try {
            PreparedStatement ps = cnx.prepareStatement(sql);
    ...
    public double getAverageRating(long destinationId) {
        String sql = "SELECT AVG(rating) as avg_rating FROM reviews WHERE destination_id = ?";
        try {
            PreparedStatement ps = cnx.prepareStatement(sql);
            ps.setLong(1, destinationId);
            ResultSet rs = ps.executeQuery();
    ...
    private void recalculateAverageRating(long destinationId) {
        double avg = getAverageRating(destinationId);
        String updateSql = "UPDATE destinations SET average_rating = ? WHERE destination_id = ?";
        try {
    ...
    private Review mapResultSetToReview(ResultSet rs) throws SQLException {
        ...
        r.setDestinationId(rs.getLong("destination_id"));
