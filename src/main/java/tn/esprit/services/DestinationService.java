...
public class DestinationService {
    ...
    public boolean addDestination(Destination destination) {
        String sql = "INSERT INTO destinations (name, type, country, city, best_season, description, timezone, average_rating, latitude, longitude, estimated_budget, popularity, image_url) " +
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        try (PreparedStatement ps = conx.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
            ...
            ps.setString(13, destination.getImageUrl());
    ...
    public boolean updateDestination(Destination destination) {
        String sql = "UPDATE destinations SET name=?, type=?, country=?, city=?, best_season=?, " +
                "description=?, timezone=?, average_rating=?, latitude=?, longitude=?, estimated_budget=?, popularity=?, image_url=? WHERE destination_id=?";
        ...
        try (PreparedStatement ps = conx.prepareStatement(sql)) {
            ...
            ps.setInt(12, destination.getPopularity() != null ? destination.getPopularity() : 0);
            ps.setString(13, destination.getImageUrl());
            ps.setLong(14, destination.getDestinationId());
    ...
    private Destination mapResultSetToDestination(ResultSet rs) throws SQLException {
        ...
        try {
            ...
            // Latitude column not found, skip
        }
        ...
        try {
            ...
            // Longitude column not found, skip
        }
