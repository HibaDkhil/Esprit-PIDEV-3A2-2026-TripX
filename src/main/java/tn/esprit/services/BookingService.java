...
public class BookingService {
    ...
    public boolean createBooking(Booking booking) {
        String sql = "INSERT INTO bookingdes (booking_reference, user_id, destination_id, activity_id, start_at, end_at, num_guests, status, payment_status, total_amount, currency, notes, user_email, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        try {
            ...
            ps.setTimestamp(14, booking.getCreatedAt());
    ...
    public boolean updateBooking(Booking booking) {
        String sql = "UPDATE bookingdes SET start_at=?, end_at=?, num_guests=?, activity_id=?, total_amount=?, notes=?, status=?, payment_status=?, user_email=?, currency=?, destination_id=? WHERE booking_id=?";
        try {
            PreparedStatement ps = cnx.prepareStatement(sql);
            ...
            ps.setString(9, booking.getUserEmail());
            ps.setString(10, booking.getCurrency());
            ps.setLong(11, booking.getDestinationId());
            ps.setLong(12, booking.getBookingId());
    ...
    private Booking mapResultSetToBooking(ResultSet rs) throws SQLException {
        ...
        try {
            ...
            if (statusStr != null) {
                b.setStatus(Booking.BookingStatus.valueOf(statusStr.trim().toLowerCase()));
            } else {
        ...
        try {
            b.setPaymentStatus(Booking.PaymentStatus.valueOf(rs.getString("payment_status").toLowerCase()));
