...
public class UserActivitiesController implements Initializable {
    ...
    private void handleBlogNav(javafx.scene.input.MouseEvent event) {
        try {
            ...
            e.printStackTrace();
            showError("Could not load blog view.");
        }
    }
    ...
    private void showActivityDetails(Activity activity) {
        ...
        reviewsBtn.setOnAction(e -> {
            try {
                ...
                reviewController.setTarget(activity.getDestinationId(), activity.getName());
    ...
    private String getCategoryColor(Activity.ActivityCategory category) {
        switch (category) {
            ...
            case Cultural: return "-fx-background-color: linear-gradient(to bottom, #9b59b6, #8e44ad); -fx-background-radius: 10 10 0 0;";
            case Relaxation: return "-fx-background-color: linear-gradient(to bottom, #3498db, #2980b9); -fx-background-radius: 10 10 0 0;";
