...
public class UserBrowsePacksController {
    ...
    private VBox createPackCard(Pack pack) {
        ...
        card.setStyle("-fx-background-color: white; -fx-background-radius: 16; -fx-padding: 20; " +
                      "-fx-border-color: #E8EAF6; -fx-border-radius: 16; -fx-cursor: hand; " +
        ...
        try {
            ...
            Offer activeOffer = offerService.getActiveOfferByPackId(pack.getIdPack());
            double offerDiscount = 0;
            if (activeOffer != null) {
                if (activeOffer.getDiscountType() == Offer.DiscountType.PERCENTAGE) {
    ...
    private void openPackDetails(Pack pack) {
        try {
            ...
            javafx.scene.layout.VBox currentView = (javafx.scene.layout.VBox) packsGrid.getScene().getRoot();
