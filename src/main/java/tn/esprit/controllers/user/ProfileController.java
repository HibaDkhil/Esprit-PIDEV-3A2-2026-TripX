...
public class ProfileController {
    ...
    private void ensureDefaultViewsMounted() {
        if (contentArea == null || personalInfoView == null || searchPreferencesView == null || securityView == null) {
            return;
        }
        if (!contentArea.getChildren().contains(personalInfoView)
                || !contentArea.getChildren().contains(searchPreferencesView)
                || !contentArea.getChildren().contains(securityView)) {
            contentArea.getChildren().setAll(personalInfoView, searchPreferencesView, securityView);
        }
    }
    ...
    private void openMyBookings() {
        try {
            ...
            showAlert("Error", "Could not open bookings page: " + e.getMessage());
        }
    }
        ...
        Thread loadThread = new Thread(() -> {
            ...
            while (retries > 0) {
                try {
                    ...
                    Image img = new Image(urlWithTimestamp, size, size, true, true, false);
                    if (img.isError()) {
                        throw new RuntimeException("Avatar image failed to load");
                    }
    ...
    private VBox createAvatarOption(String style, String seed) {
        ...
        Thread loadThread = new Thread(() -> {
            try {
                Image img = new Image(avatarUrl, 70, 70, true, true, false);
                if (img.isError()) {
                    throw new RuntimeException("Preview avatar failed to load");
                }
                Platform.runLater(() -> {
                    ImageView avatarView = new ImageView(img);
