...
public class TransportUserInterfaceController {
    ...
    private void buildSchedulesView() {
        ...
        List<Transport> allTrans = transportService.getActiveTransports();
        ...
        List<Schedule>[] displayed = new List[]{
                allSched.stream()
                        ...
                        .filter(s -> transMap.containsKey(s.getTransportId()))
    ...
    private void buildProviderList(String type) {
        List<Transport> typed = transportService.getActiveTransports().stream()
    ...
    private void buildTransportCards(String type, String provider) {
        List<Transport> list = transportService.getActiveTransports().stream()
    ...
    private void showTransportDetail(Transport t) {
        ...
        transportService.getActiveTransports().forEach(tr -> tMap.put(tr.getTransportId(), tr));
    ...
    private void openBookingForm(Transport transport, Schedule schedule, Stage ownerStage) {
        Map<Integer, Transport> tMap = new HashMap<>();
        transportService.getActiveTransports().forEach(t -> tMap.put(t.getTransportId(), t));
        ...
        if (!resolvedT.isActive()) {
            showAlert("This transport is no longer available.");
            return;
        }
