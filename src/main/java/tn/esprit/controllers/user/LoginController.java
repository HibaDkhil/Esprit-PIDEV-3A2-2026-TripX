...
public class LoginController {
    ...
    private void handleLogin() {
        ...
        if (loggedInUser != null) {
            ...
            try {
                // Normalize PHP BCrypt prefix ($2y$) to Java prefix ($2a$)
    ...
    private void handleFacebookLogin() {
        ...
        alert.setHeaderText("Facebook Login");
        alert.setContentText("Facebook login will be available soon!");
        alert.showAndWait();
    }
    ...
    private void showGooglePopup() {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/fxml/user/oauth_popup.fxml"));
            Parent root = loader.load();

            OAuthPopupController controller = loader.getController();

            Stage popupStage = new Stage();
            popupStage.setTitle("Login with Google");
            popupStage.setScene(new Scene(root));
            popupStage.setResizable(false);
            popupStage.initModality(javafx.stage.Modality.APPLICATION_MODAL);

            controller.startGoogleOAuth(new OAuthPopupController.OAuthCallback() {
                @Override
                public void onSuccess(String email, String name) {
                    handleOAuthSuccess(email, name);
                }
                ...
                public void onFailure(String error) {
                    ...
                    alert.setHeaderText("Google Login Error");
            ...
            alert.setHeaderText("Could not open login window");
            alert.setContentText(e.getMessage());
            alert.showAndWait();
        }
    }

    private void showLinkedInPopup() {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/fxml/user/oauth_popup.fxml"));
            Parent root = loader.load();
            ...
            Stage popupStage = new Stage();
            popupStage.setTitle("Login with LinkedIn");
            popupStage.setScene(new Scene(root));
            popupStage.setResizable(false);
            popupStage.initModality(javafx.stage.Modality.APPLICATION_MODAL);

            controller.startLinkedInOAuth(new OAuthPopupController.OAuthCallback() {
                @Override
                public void onSuccess(String email, String name) {
                    handleOAuthSuccess(email, name);
                }
                ...
                public void onFailure(String error) {
                    Alert alert = new Alert(Alert.AlertType.ERROR);
                    alert.setTitle("Login Failed");
                    alert.setHeaderText("LinkedIn Login Error");
                    alert.setContentText(error);
                    alert.showAndWait();
                }
            });
    ...
    private void handleOAuthSuccess(String email, String name) {
        UserService userService = new UserService();
        User user = userService.findByEmail(email);

        if (user == null) {
            // User doesn't exist - ask to create account
            Alert alert = new Alert(Alert.AlertType.CONFIRMATION);
            ...
            Button yesButton = (Button) alert.getDialogPane().lookupButton(ButtonType.YES);
            yesButton.setStyle("-fx-background-color: #4cccad; -fx-text-fill: white;");
            ...
            if (result.isPresent() && result.get() == ButtonType.YES) {
                ...
                if (name.contains(" ")) {
                    lastNameField.setText(name.substring(name.indexOf(" ") + 1));
                }
        ...
        // User exists - login
        tn.esprit.utils.SessionManager.setCurrentUserId(user.getUserId());
        String role = userService.getRoleByEmail(email);
